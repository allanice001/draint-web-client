...
export default function AustralianContinents(props) {
  const {
    oceania,
    totalPages,
    currentPage,
  ...
  const totalArtwork = oceania.length;
  if (totalArtwork === 0) return null;
  return (
    <>
      {oceania.length && (
        <div className="card-wrapper-continents">
          <div className="filtered-header-new">AUSTRALIAN CONTINENT</div>
          ...
          {oceania.map(artist => (
            <Card key={`ocean-artist-${artist.id}`} className="card-home">
              <CardActionArea>
                <Link
                  to={
                    artist.username
