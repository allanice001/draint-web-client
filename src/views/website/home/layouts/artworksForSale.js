...
export default function ArtworksForSale(props) {
  const { artworksForSale } = props;
  const totalArtwork = artworksForSale.length;
  if (totalArtwork === 0) return null;
  return (
    <>
      <div className="filtered-header-sale">ARTWORKS UP FOR SALE</div>
      <div className="artworks-wrapper-container-home">
        {artworksForSale.map(artwork => (
          <Card key={`artworks-for-sale-${artwork.id}`} className="card">
            <CardActionArea>
              <Link
