...
import { getShipmentSectionsFields } from 'constants/components/homepage';
import styles from '../homepage.module.scss';
import { useSelector } from 'react-redux';

const EditSectionForm = ({ invalid, handleSubmit }) => {
  const { values = {} } = useSelector(
    state => state.form.updateShipmentSections
  );

  const form = useMemo(() => {
    const fieldList = getShipmentSectionsFields(values.use_artwork);
